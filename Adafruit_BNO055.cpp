...
bool Adafruit_BNO055::getSensorRawData(double &ax, double &ay, double &az,
    ...
    ax = (int16_t)((buffer[0]) | (buffer[1] << 8)) / 100.0; // Convert to m/s²
    ay = (int16_t)((buffer[2]) | (buffer[3] << 8)) / 100.0;
    az = (int16_t)((buffer[4]) | (buffer[5] << 8)) / 100.0;

    // Parse magnetometer (6 bytes) - 1 LSB = 16 µT
    mx = (int16_t)((buffer[6]) | (buffer[7] << 8)) / 16.0; // Convert to µT
    my = (int16_t)((buffer[8]) | (buffer[9] << 8)) / 16.0;
    mz = (int16_t)((buffer[10]) | (buffer[11] << 8)) / 16.0;

    // Parse gyroscope (6 bytes) - 1 LSB = 1 dps
    gx = (int16_t)((buffer[12]) | (buffer[13] << 8)) / 16.0; // Convert to °/s
    gy = (int16_t)((buffer[14]) | (buffer[15] << 8)) / 16.0;
    gz = (int16_t)((buffer[16]) | (buffer[17] << 8)) / 16.0;
 ...
 */
bool Adafruit_BNO055::getSensorExtendedData(double &lin_x, double &lin_y, double &lin_z,
    ...
    grav_x = (int16_t)(buffer[6] | (buffer[7] << 8)) / 100.0; // Convert to m/s²
    grav_y = (int16_t)(buffer[8] | (buffer[9] << 8)) / 100.0;
    grav_z = (int16_t)(buffer[10] | (buffer[11] << 8)) / 100.0;
